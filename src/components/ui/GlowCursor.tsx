...
/**
 * GlowCursor（React Bits）· shader 驱动的跟随光轨
 * ============================================================================
 * 来源：reactbits.dev 的 GlowCursor 组件，JavaScript + CSS 变体
 ...
 *      CSS 也是官方 registry 里的原文。
 *
 * 它本身的几个好性质（所以本站不需要再包一层）：
 *   - canvas 是 `pointer-events: none`，**不隐藏原生指针、不拦截点击**；
 *   - `idleFade`：指针离开或静止超过 idleTimeout 后光轨自动淡出；
 *   - 卸载时断开所有监听并销毁 ogl 资源。
 *
 * **它没有处理 prefers-reduced-motion，也没有区分触屏。** 这两条由调用方
 * 通过 `enabled` 控制，见 src/components/layout/CursorGlow.tsx。
 *
 * 性能提醒：fragment shader 里对每个像素固定循环 MAX_POINTS-1 = 63 次，
 * 与 trailLength 无关。所以开销**只由 canvas 面积与 dpr 决定**：
 * 要降开销就缩小包裹范围，或调低 maxDevicePixelRatio。
 */
...
export interface GlowCursorProps
  ...
  /** 光轨尾部颜色 */
  secondaryColor?: string
  /** 光轨点数，2 到 64。注意：**不影响 GPU 开销**，shader 的循环是固定 63 次 */
  trailLength?: number
  trailWidth?: number
  trailTaper?: number
  followSpeed?: number
  ...
  fadeDuration?: number
  blendMode?: CSSProperties['mixBlendMode']
  /** canvas 像素密度上限，直接影响 GPU 开销 */
  maxDevicePixelRatio?: number
  /** 关掉时淡出，是控制触屏 / 降动效偏好的开关 */
  enabled?: boolean
  children?: ReactNode
}
